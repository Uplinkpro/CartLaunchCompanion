...
public static class LaunchService
{
    public static void Launch(GameDefinition game)
    {
        ...
        if (launcher.Equals("Xbox", StringComparison.OrdinalIgnoreCase))
        {
            LaunchXboxApp(target);
            return;
        }
        ...
        if (process is null)
        {
            throw new InvalidOperationException(
                $"Windows did not accept the launch target: {target}");
        }
    }

    private static void LaunchXboxApp(string target)
    {
        ...
        // Xbox and Microsoft Store games are handed off to Explorer.
        // Explorer may not return the launched game's Process object, so
        // a successful handoff is determined by the absence of an exception.
    ...
    public static string NormalizeLauncher(string? launcher)
    {
        ...
        if (value.Equals("Xbox Game Pass", StringComparison.OrdinalIgnoreCase) ||
            ...
            value.Equals("Microsoft Store", StringComparison.OrdinalIgnoreCase))
            return "Xbox";
        ...
        if (value.Equals("Ubisoft Connect", StringComparison.OrdinalIgnoreCase) ||
            value.Equals("Uplay", StringComparison.OrdinalIgnoreCase))
            return "Ubisoft";
